...
public class AdminTableViewDoctor implements Initializable {
    ...
    private void generatePdf() {
        ...
        try {
            PdfWriter.getInstance(document, new FileOutputStream("DoctorTable.pdf"));
            document.open();

            PdfPTable pdfTable = new PdfPTable(tableView.getColumns().size() - 2); // Exclude switch and block columns

            // Add column headers to the PDF table, excluding switch and block columns
            for (TableColumn<?, ?> column : tableView.getColumns()) {
                if (!column.getText().equals("Block") && !column.getText().equals("Delete") ) {
            ...
            ObservableList<Doctor> patients = tableView.getItems();
            for (Doctor patient : patients) {
                ...
                pdfTable.addCell(patient.getSpecialite());
                pdfTable.addCell(patient.getAddress());
            }
